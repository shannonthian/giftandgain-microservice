  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        {targetInventoryList && targetInventoryList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
            ...
            <tbody>
              {targetInventoryList.map((targetInventory, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/target-inventory/${targetInventory.id}`} color="link" size="sm">
                      {targetInventory.id}
                    </Button>
                  </td>
                  <td>{targetInventory.itemName}</td>
                  <td>{targetInventory.quantity}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button
                        tag={Link}
                        to={`/target-inventory/${targetInventory.id}`}
                        color="info"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/target-inventory/${targetInventory.id}/edit`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/target-inventory/${targetInventory.id}/delete`}
                        color="danger"
                        size="sm"
